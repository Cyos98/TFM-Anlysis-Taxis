...
from nyc_taxi_pipeline.config import AppConfig, SUPPORTED_SERVICES
...
def train_demand_models(config: AppConfig, mode: str = "demo") -> dict[str, object]:
    ...
    ControlRepository(config.database).apply_migrations(config.paths.sql_root)
    ...
    artifacts_root = Path("/app/ml/artifacts")
